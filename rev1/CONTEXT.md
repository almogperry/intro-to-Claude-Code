...
**Task**:
A unit of work tracked on the board. Each task belongs to exactly one [[column]] (its current stage) and carries a [[category]]. A task is considered finished when it sits in a [[terminal column]].
...
**Terminal column**:
A column flagged as representing completion. A task in a terminal column is considered finished — it sinks to the bottom of the column and is marked with a large ✓. By default the seeded "Completed" column is the only terminal column; the user may flag additional columns terminal or unflag the default. Moving a task into a terminal column while it has open [[subtask]]s triggers a confirmation prompt.
...
**Due-window**:
The period before a task's due date during which the task is visually framed as "upcoming". Width is determined by the task's [[scope]] (3h / 3d / 1w / 3mo for day / week / month / year).
...
>
> **PM**: That's a [[terminal column]] by default, so the card sinks to the bottom of Completed and gets a large ✓ on it. If it has open [[subtask]]s, you get a confirm prompt first.
>
> **Dev**: What if I rename "Completed" to "Shipped"?
>
> **PM**: Same column, still flagged terminal — behaviour unchanged. The flag is what matters, not the name.
>
> **Dev**: And the [[category]] — that's like a tag?
>
> **PM**: No — one category per [[task]], picked from a user-defined list. Tags are multi-select; categories aren't. Defaults are "personal" and "work/study".
